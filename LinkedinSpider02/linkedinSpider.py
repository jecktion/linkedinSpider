...
def get_linkedin_url(url, s):
    ...
    try:
        ...
        print ('get linkedin url 失败: %s' % url)
    return ''
...
def parse(content, url):
    ...
    if firstname and lastname:
        Linkedin = url
        print ('姓名: %s%s    Linkedin: %s' % (lastname[0], firstname[0], Linkedin))
...
def crawl(url, s):
    ...
    try:
        ...
        if len(url) > 0 and url not in LINKS_FINISHED:
            ...
            #每个员工的url
            # print('LINKS_FINISHED:',LINKS_FINISHED)
            ...
            while failure < 10:
                ...
                if r.status_code == 200:
                    print('搜索成功的: %s %s' % (r.status_code, url))
                    parse(r.content, url)
                    break
                else:
                    ...
                    failure += 2
            if failure >= 10:
                print ('Failed 失败的: %s' % url)
    except Exception as e:
        pass


if __name__ == '__main__':
    s = login(laccount='13439157224', lpassword='wgs7968463mmm')  # 测试账号
    company_name = input('Input the company you want to crawl:')
    maxpage = 1  # 抓取前50页百度搜索结果，百度搜索最多显示76页
    ...
    while len(url) > 0 and failure < 10:
        ...
        if r.status_code == 200:
            #百度搜索linkedin员工页列表:
            hrefs = list(set(re.findall(r'"(http://www\.baidu\.com/link\?url=.*?)"', r.content.decode('utf-8'))))  # 一页有10个搜索结果
            # print('hrefs:',hrefs)
            # print(len(hrefs))
            for href in hrefs:
                # 百度搜索linkedin员工页
            ...
            # print('nextpage_txt:',nextpage_txt)
            #百度搜索下一页链接
            ...
            failure += 2
            print('search failed 搜索失败: %s' % r.status_code)
    if failure >= 10:
        print ('search failed 搜索失败: %s' % url)
