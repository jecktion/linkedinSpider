...
def parse(content, url):
    ...
    """ 解析一个员工的Linkedin主页 """
    time.sleep(20)
    # content = urllib.parse.unquote(content).replace('&quot;', '"')
...
def runSpider(username = '', password = '', company =''):
    ...
    # 百度搜索
    url = 'http://www.baidu.com/s?ie=UTF-8&wd=%20%7C%20领英%20' + urllib.parse.quote(company_name) + '%20site%3Alinkedin.com'
    results = []
    failure = 0
    while len(url) > 0 and failure < 10:
        ...
        if r.status_code == 200:
            ...
            nextpage = u'下一页>'
            nextpage_txt = tree.xpath('//div[@id="page"]/a[@class="n" and contains(text(), "%s")]/@href' % (nextpage))
...
class APP:
    def __init__(self, width=550, height=350):
        self.w = width
        self.h = height
        self.title = ' LinkedIn数据爬虫'
        ...
        filemenu.add_command(label='使用说明', command=lambda: msgbox.showinfo(title='使用说明', message='根据公司名称抓取员工Linkedin数据'))
        filemenu.add_command(label='关于作者', command=lambda: msgbox.showinfo(title='关于作者', message='大漠孤烟wgs'))
        filemenu.add_command(label='退出', command=self.root.quit)


        # 控件内容设置
        group0 = tk.Label(frame_1, text='请选择一个下载模式：',font=('宋体', 11), padx=10, pady=10)
        tb1 = tk.Radiobutton(frame_1, text='普通模式', variable=self.v, value=1, font=('宋体', 11),width=10, height=3)
        tb2 = tk.Radiobutton(frame_1, text='强力模式', variable=self.v, value=2, font=('宋体', 11),width=10, height=3)
        group = tk.Label(frame_1, text='请输入用户名：', font=('宋体', 11),padx=10, pady=10)
        un = tk.Entry(frame_1, textvariable=self.username, bd=3, bg = '#CCCCCC', font=('宋体', 13), highlightcolor='Fuchsia', highlightthickness=1, width=30)
        group2 = tk.Label(frame_1, text='请输入密码：', font=('宋体', 11),padx=10, pady=10)
        pw = tk.Entry(frame_1, textvariable=self.password,  bd=3, bg = '#CCCCCC', font=('宋体', 13), highlightcolor='Fuchsia', highlightthickness=1, width=30,show='*')
        label1 = tk.Label(frame_2, text="请输入公司名称：",font=('宋体', 11),)
        entry = tk.Entry(frame_2, textvariable=self.url,  bd=3, bg = '#CCCCCC', font=('宋体', 13), highlightcolor='Fuchsia', highlightthickness=1, width=35)
        label2 = tk.Label(frame_2, text=" ")
        play = tk.Button(frame_2, text="开始", font=('宋体', 16), bd=5, fg='Purple',bg='#CCCCCC', width=2, height=1, command=self.start)
        label3 = tk.Label(frame_2, text=" ")
        label_proccess = tk.Label(frame_3, textvariable=self.proccess, fg='blue', font=('宋体', 12), text='结束')
        label_explain = tk.Label(frame_3, fg='#990000', font=('宋体', 12),
                                 text='\n注意：请在表单中输入您的领英帐号!')
        label_warning = tk.Label(frame_3, fg='#990099', font=('宋体', 12), text='\n温馨提示：账号被封, 概不负责!')
